...
for i in arange(N_t) :
	ik = (i+1)*1
	Omega = np.load(path+'/PV'+str(ik)+'.0.npy');
	psi_n = InvPotentialVorticitySine(Omega);
	u_n = -partialYSine(psi_n);
	v_n = partialX(psi_n);
...
Omega = np.load(path+'/PV'+str(1)+'.0.npy');
psi_n = InvPotentialVorticitySine(Omega);
figure(figsize= (15,6))
subplot(121)
...
Omega = np.load(path+'/PV'+str(36)+'.0.npy');
psi_n = InvPotentialVorticitySine(Omega);
figure(figsize= (15,6))
subplot(121)
imshow(Omega.transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
