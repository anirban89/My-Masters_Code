...
omega_b = partialX(si,2) + partialYCosine(si,2) - si ;

#for i in arange(N):
#    omega[i,:] = cos(2*x[i])*cos(2*y)*exp(-pow(y-pi/2,2));


omega = omega_b*exp(-pow(y-pi/2,2));
...
def domegadt(dt, omega, args):
    ...
    return real(-(u*partialX(omega) + v*partialYCosine(omega) + Beta*v + forcing));


def calcMax_omega(fields, args):
    omegaNew = fields;

    psi = InvPotentialVorticityCosine(omegaNew);
...
#    return fields;
    ...
    for i in arange(leny-1):
        omegaNew[:,newLen-i-1] = fields[:,i+1];
...
subplot(133)
handle3 = imshow(-partialYCosine(psi).transpose(), vmin=-15,vmax=1, aspect='auto');
c4 = colorbar();
draw();
...
while (sec<10000):
	...
	handle2.autoscale();
        handle3.set_array(-partialYCosine(psi).transpose());
        handle3.autoscale();
        sec = sec+1;
        draw();
