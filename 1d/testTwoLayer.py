...
if (alphaVapor < 0):
    path = home+'/baroclinic/TwoLayerNegative';
else:
    path = home+'/baroclinic/TwoLayer';
...
vapor = 0.0*rand(N,N);
slope = -0.01;
for j in arange(N/4,N/3):
        psi2[:,j] = 0.1*(j-N/4)*h;

for j in arange(N/3,2*N/3):
        psi2[:,j] = 0.1*(N/3-N/4)*h;

for j in arange(2*N/3,3*N/4):
        psi2[:,j] = psi2[:,N/3] - 0.1*(j-2*N/3)*h;
#for i in arange(N):
#    for j in arange(N):
#        vapor[i,j] = \
#                 3*exp(-(pow((i-N/2)*h,2)+pow((j-N/2)*h,2))/0.25);
		 #slope*j*h;
#                0.01*cos(2*pi*(N/2-i)*2/float(N))*sin(2*pi*(j)*3/float(N));
#                 0.1*exp(-pow((j-N/2),2)/500.0) ;

#                0.1*cos(2*pi*(N/2-i)*1/float(N))*sin(2*pi*(j)*0.5/float(N));
...
def dudt_vapor(dt, vapor, args):
    ...
    return real(-(u*partialX(vapor) + v*partialY(vapor) -betaQ*v + alphaQ*u + delta));
...
handle2 = imshow((SF1-SF2).transpose(), vmin=-15,vmax=1, aspect='auto');
c4 = colorbar();
#draw();
...
while (sec<5000):
    ...
    if tn>forcingTime:
        ...
        forcing = 0.00*ones((N,N));
    
    psi1,psi2 = deriveVelocities(omega1_n, omega2_n);

    vaporContent = qs + vapor_n;

    v = partialX(psi2);
    u = -partialY(psi2);

    condensing = vapor_n;
    #condensing[condensing<0] = 0;

    deltaSouth = abs(qs[0,:] - qs[N-1,:]);

    sourceSouth = sum(v[N-1,:]*partialX(vaporContent)[N-1,:]*dt);
    #sourceSouth = sum(v[N-1,:]*partialX(vaporContent)[N-1,:]*dt + deltaSouth);


    deltaEast = abs(qs[:,0] - qs[:,N-1]);

    #sourceEast = sum(-u[N-1,:]*partialY(vaporContent)[N-1,:]*dt + deltaEast);
    sourceEast = sum(-u[:,N-1]*partialY(vaporContent)[:,N-1]*dt);

    print 'Total Source: ',sourceEast+sourceSouth;
    ...
    if(tn > sec):
        ...
        SF1,SF2 = deriveVelocities(omega1_n,omega2_n);
        handle2.set_array(SF1.transpose());
        handle2.autoscale();
        handle1.set_array(SF2.transpose());
        handle1.autoscale();
