...
for i in arange(5*N/16,9*N/16):
    for j in arange(N):
        n[i,j] = \
        0.001*exp(-pow(y[j]-3*pi,2)/4*pi)*cos(x[i]);
...
def dudt(dt, omega, args):
    ...
    u_n = real(-(u*boydPartialX(u) + v*boydPartialY(u,bc) + boydPartialX(n)) + vy/(4*pi));
#    u_n = real(-u*boydPartialX(u) - v*boydPartialY(u,bc) - 10*boydPartialX(n) + 0.001*vy);
...
def dvdt(dt, omega, args):
    ...
    v_n = real(-(u*boydPartialX(v) + v*boydPartialY(v, bc) + boydPartialY(n, bc) + uy/4*pi) );
#    v_n = real(-u*boydPartialX(v) - v*boydPartialY(v, bc) - 10*boydPartialY(n, bc) - 0.001*uy);
...
#    n_n = real(-(boydPartialX(u) + boydPartialY(v, bc)));
    n_n = real(-(boydPartialX(n*u) + boydPartialY(n*v, bc) + (boydPartialX(u) + boydPartialY(v, bc))));
#    n_n = real(-(boydPartialX(n*u) + boydPartialY(n*v,bc) + (boydPartialX(u) + boydPartialY(v,bc))) );
...
#        c2.update_bruteforce(handle2);
    if(tn > sec):
            savefig('/home/joy/EqBetaG/fig'+str(sec)+'.png');
            np.save('/home/joy/EqBetaG/eta_data_'+str(sec)+'.npy',n.transpose());
            np.save('/home/joy/EqBetaG/uVel_data_'+str(sec)+'.npy',u.transpose());
            np.save('/home/joy/EqBetaG/vVel_data_'+str(sec)+'.npy',v.transpose());
            sec = sec+1;
    draw();
