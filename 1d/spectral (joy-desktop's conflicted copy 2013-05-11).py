from scipy.fftpack import fft, ifft, ifftshift,fft2,ifft2, dct, idct;
from scipy import *;
from pylab import *;
...
def InvPotentialVorticitySine(field, length=None):
    ...
    ky = concatenate((range(0,newLen/2),range(-newLen/2,0)));
    kx = k * (2*pi)/length;
    ky = ky * (2*pi)/(length);
...
#    return idct(temp, type=1,axis=0,norm='ortho')/(float(halfLength));
