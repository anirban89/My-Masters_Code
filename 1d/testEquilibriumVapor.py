...
resume = 0;
path = '/home/joy/EqVaportestHiRes';
...
for i in arange(N/2):
    qs[i] = q0 - beta*dy*(N/2-i);
    qs[N-i-1] = q0 - beta*dy*(N/2-i);
...
#    delta[etaDiff > eta_u] = 0;
    ...
    '''
    delta_eta = delta*vapor_n;
    print 'forcing strength: ',sum(delta_eta);
...
#    print 'minimum vapor value: ', amin(amin(vapor));
    ...
    if(tn > sec):
        ...
        if(forcingTime > 5):
            ...
            for j in arange(N):
                for i in arange(N):
                    vapor_n[i,j] = vapor_n[i,j] + (qs[N/2]-qs[0])/100;
 ...
 #                       vapor_n[i][j] = 0;
       ...
       # c1.update_bruteforce(handle);
        np.save(path+str(coeff)+'/rh_data_'+str(sec)+'.npy',rh.transpose());
        np.save(path+str(coeff)+'/vapor_data_'+str(sec)+'.npy',vapor_n.transpose());
        np.save(path+str(coeff)+'/delta_data_'+str(sec)+'.npy',delta.transpose());
