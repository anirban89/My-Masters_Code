...
resume = 0;
#path = '/home/joy/EqVaportestJet';
path = '/Data/EqVaportestJet';
...
for i in arange(3*N/20,9*N/20):
    omega[:,i] = jetStrength*i;

for i in arange(9*N/20, 11*N/20):
    omega[:,i] = jetStrength*7*N/20;

for i in arange(11*N/20, 17*N/20):
    omega[:,i] = jetStrength*7*N/20 - jetStrength*(i-13*N/20);
...
#    delta = 1/(1+exp(-1000*vapor_new));
    ...
    dVapor = dudt_vapor(dt, vapor_new, (u,v,coeff2,delta));
...
while (sec<20000):
    ...
    condensationCount = 0;
    negativeRHCount = 0;
    (dt,fields) = stepper.integrate(tn,[omega_n, vapor_n],(coeff));
...
#    print 'minimum vapor value: ', amin(amin(vapor));
 ...
 #                   vapor_n[i,j] = 0;
            ...
            vapor_n = vapor_n + (qs[N/2]-qs[0])/1000;
#
            print "Vapor added ";
            forcingTime = 0;
