...
#        omega[i,j] = exp(-(pow(i-N/2,2)+pow(j-N/4-2,2))/200);
...
def dudt_omega(dt, omega, args):
    ...
    """
    delta = args;
    psi = InvPotentialVorticitySine(omega);
    u = -partialYSine(psi);
    v = partialX(psi);

    return real(-(u*partialX(omega) + v*partialYSine(omega) + v + delta*psi));
...
#   delsq = -(KX*KX + KY*KY)
    ...
    delsq = (nu*delsq - mu)*dt;
    multiplier = exp(delsq);
    omegaHat = omegaHat*multiplier;
    return real(ifft2(omegaHat)[:,1:leny+1]);
...
def dudt_vapor(dt, vapor, args):
    ...
    return real(-(u*partialX(omega) + v*partialYSine(omega) + beta*v));
...
sfn = InvPotentialVorticitySine(omega);
u = -partialYSine(omega);
v = partialX(omega);
...
while (1):
   ...
   # psi[psi<-eta_u] = -eta_u;
    ...
    ''' Force boundary conditions: equator always saturated'''
    vapor_n[:,0] = qs[0];
    ...
    condensationCount = 0;
    negativeRHCount = 0;
    vorticity = partialX(v) - partialYSine(u);
...
#        np.save('/home/joymm/VaporS/vapor_data_'+str(sec)+'.npy',vapor_n.transpose());
        handle3.set_array(delta.transpose());
        handle3.autoscale();
#        np.save('/home/joymm/VaporS/eta_data_'+str(sec)+'.npy',omega_n.transpose());
        ...
        handle5.autoscale();
        sec = sec+1;
        savefig('/home/joymm/VaporS/fig'+str(sec)+'.png');
        draw();
