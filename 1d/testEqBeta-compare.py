...
diff = coupledDerivative(N,N,2*pi,2*pi);

bc1 = boundary(yBcType='Dirichlet',bcs=(zeros((N,10)),zeros((N,10))));
...
figure(figsize= (18,6));
subplot(131);
handle_u = imshow(zeros((N,N)),vmin=-0.1,vmax=0.1);
c_u = colorbar();
draw();

subplot(132);
handle_v = imshow(zeros((N,N)),vmin=-0.1,vmax=0.1);
c_v = colorbar();
draw();

subplot(133);
handle_n = imshow(zeros((N,N)),vmin=-0.1,vmax=0.1);
c_n = colorbar();
draw();
...
def dudt(dt, omega, args):
    ...
    bpx = boydPartialY(u,bc);
    px = diff.partialX(u,bc1);


    handle_v.set_array(bpx.transpose());
    handle_v.autoscale();

    handle_n.set_array(px.transpose());
    handle_n.autoscale();

    handle_u.set_array((bpx-px).transpose());
    handle_u.autoscale();
    raw_input();
