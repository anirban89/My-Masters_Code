...
def dndt(dt, omega, args):
    ...
    n_n = real(-(partialX(n*u) + partialY(n*v) + (partialX(u) + partialY(v))*epsInv - delta*n) );
...
#    print 'forcing strength: ',sum(delta);
    ...
    (tn2,omega_n) = stepper_omega.integrate(prev_t,omega_n,(delta_eta), 0);
    DT = tn2 - prev_t;
    prev_t = tn2;
    (u,v,n) = omega_n;
    print 'Time elapsed: ',tn2;
