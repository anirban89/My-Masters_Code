...
def dudt(dt, omega, args):
    ...
    u_n = real(-(u*boydPartialX(u,16*pi) + v*boydPartialY(u,8*pi) + boydPartialX(n,16*pi) + d*u/epsilon) + vy);
...
def dvdt(dt, omega, args):
    ...
    v_n = real(-(u*boydPartialX(v,16*pi) + v*boydPartialY(v,8*pi)) - (boydPartialY(n,8*pi) + uy)/epsSq - d*u/epsSq);
...
def dndt(dt, omega, args):
    ...
    n_n = real(-(boydPartialX(h*u,16*pi) + boydPartialY(h*v,8*pi) + boydPartialX(u,16*pi) + boydPartialY(v,8*pi) + d*n/epsilon) + Q);
