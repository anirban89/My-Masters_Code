...
for j in arange(N):
        Q[j] = 0.01*exp(-pow(y[j]-4*pi,2)/(4*pi));
...
def dudt(dt, omega, args):
    ...
    u_n = real(-v*boydPartialY(u, bc, 8*pi) + vy/(4*pi));
...
def dndt(dt, omega, args):
    ...
    n_n = real(-(boydPartialX(u,16*pi) + boydPartialY(v, bc, 8*pi) + epsilon*n + Q));
...
stepper_ = integrator(h, [dudt,dvdt,dndt], [diffusion_SWE], calcMax_omega, dim=3);
...
#        c2.update_bruteforce(handle2);
    if(tn > sec):
            savefig('/home/joy/EqBeta-1/fig'+str(sec)+'.png');
            np.save('/home/joy/EqBeta-1/eta_data_'+str(sec)+'.npy',n.transpose());
            np.save('/home/joy/EqBeta-1/uVel_data_'+str(sec)+'.npy',u.transpose());
            np.save('/home/joy/EqBeta-1/vVel_data_'+str(sec)+'.npy',v.transpose());
            sec = sec+1;
    draw();
