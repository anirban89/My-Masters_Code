...
for i in arange(15*N/16, 17*N/16):
    for j in arange(N):
        Q[i,j] = cos(x[i])*exp(-pow(y[j]-4*pi,2)/4);
...
def dudt(dt, omega, args):
    ...
    u_n = real(-(diff.partialX(n) + epsilon*u) + vy/(4*pi));
...
def dvdt(dt, omega, args):
    ...
    v_n = real(-(diff.partialY(n, bc) + epsilon*v) - uy/(4*pi));
...
def dndt(dt, omega, args):
    ...
    n_n = real(-(diff.partialX(u) + diff.partialY(v, bc) + epsilon*n + Q));
...
#    print 'forcing strength: ',sum(delta);
    ...
    (DT,omega_n) = \
    stepper_omega.integrate(prev_t,omega_n,(bc, deriv), 0);
    prev_t = tn;
    tn = tn + DT;
    #DT = tn2 - prev_t;
    #tn = tn + DT;
