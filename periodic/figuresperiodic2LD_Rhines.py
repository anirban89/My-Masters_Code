...
title(r' Streamfunction field, $\psi$  at t ='+str(tx),fontsize=16)
xlabel(' x ',fontsize=16)

ylabel(' y ',fontsize=16)

colorbar()
savefig('/home/anirban/SymJetsIC-Strfn-LDinf-Beta-'+str(Beta)+'-t-'+str(tx)+'.png');

figure(figsize= (9,6))
imshow(Uf,aspect='auto',extent=(0,2*pi,0,2*pi));
title(r'Zonal Velocity, $U$ at t ='+str(tx),fontsize=16)
xlabel(' x ',fontsize=16)

ylabel(' y ',fontsize=16)

colorbar()
savefig('/home/anirban/SymJetsIC-U-LDinf-Beta-'+str(Beta)+'-t-'+str(tx)+'.png');
...
Omega = np.load(path1+'/PV'+str(tx)+'.npy');
psi_n = InvPotentialVorticity(Omega);
psi_nf = flipud(psi_nf.transpose())
U = -partialY(psi_n);
Uf = flipud(U.transpose());
Uyy = partialY(U,2);
