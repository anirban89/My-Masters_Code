...
savefig('/home/joymm/anirban/Vars_LD'+str(LD)+'Beta-'+str(Beta)+'.png');
...
colorbar()
savefig('/home/joymm/anirban/fields_LD'+str(LD)+'Beta-'+str(Beta)+'t-1.png');

Omega = np.load(path+'/PV'+str(100)+'.npy');
psi_n = InvPotentialVorticity(Omega);
figure(figsize= (15,6))
subplot(121)
imshow(Omega.transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
title("PV field at t = 100",fontsize=16)
colorbar()
subplot(122)
imshow(psi_n.transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
title(" Streamfunction field at t = 100",fontsize=16)
colorbar()
savefig('/home/joymm/anirban/fields_LD'+str(LD)+'Beta-'+str(Beta)+'t-100.png');
show();
