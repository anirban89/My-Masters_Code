...
path = home+'/PVRampPeriodicLDinfBeta5';
path2 = home+'/PVRampPeriodicLDinfBeta0';
#LD = 1;
...
Omega = np.load(path+'/PV'+str(500)+'.npy');
psi_n = InvPotentialVorticity(Omega);
U = -partialY(psi_n);
Uyy = partialY(U,2);
...
imshow(psi_n.transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
title(" Streamfunction field at t = 500",fontsize=16)
colorbar()


#imshow(Omega.transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
#title("PV field at t = 500",fontsize=16)
#colorbar()
...
imshow((Uyy-Beta).transpose(),aspect='auto',extent=(0,2*pi,0,2*pi));
title(" Rayleigh-Kuo Criterion  t = 500",fontsize=16)
colorbar()
