...
omega1_p = partialX(partialX(psi_p)) + partialY(partialY(psi_p)) - psi_p ;
for j in arange(N):
	omega_p[:,j] = omega1_p[:,j] + Beta*y[j];
"""
BASE FLOW
"""
for i in arange(N):
    si[:,i] = 2*(arctan(tanh(ax*(Lk - y1[i]))))/bx + 2*(arctan(tanh(ax*(Lk + y1[i]))))/(ax*bx)
omega1 = partialX(partialX(si)) + partialY(partialY(si)) - si ;
for j in arange(N):
	omega[:,j] = omega1[:,j] + Beta*y1[j];
       
omega = omega + 0.001*omega_p;       
"""
for i in arange(N):
    for j in arange(N):
